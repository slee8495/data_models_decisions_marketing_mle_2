...
poisson_regression_likelihood <- function(beta, Y, X) {
  lambda <- exp(X %*% beta)  # Calculate lambda for each observation
...
poisson_regression_likelihood <- function(beta, Y, X) {
  eta <- X %*% beta
  lambda <- exp(pmin(eta, 20))  # Cap the exponential to avoid overflow
  -sum(dpois(Y, lambda, log = TRUE))  # Negative for minimization in 'optim'
}
